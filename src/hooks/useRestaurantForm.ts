...
export function useRestaurantForm() {
  ...
  const [coverImage, setCoverImage] = useState<{ file: File | null; preview: string }>({
    file: null,
    preview: "",
  });
  ...
  const validateStep = (step: number): { isValid: boolean; errors: string[] } => {
    ...
    switch (step) {
      case 1:
        if (!formData.name.trim()) errors.push("Nome do restaurante é obrigatório");
        if (!formData.slug.trim()) errors.push("URL do restaurante é obrigatória");
      ...
      case 3:
        ...
        if (!hasOpenDay) errors.push("Pelo menos um dia da semana deve estar aberto");
        break;
      
      case 4:
        if (!formData.deliveryFee.trim()) errors.push("Taxa de entrega é obrigatória");
        if (!formData.minimumOrder.trim()) errors.push("Pedido mínimo é obrigatório");
        if (formData.paymentMethods.length === 0) errors.push("Pelo menos uma forma de pagamento deve ser selecionada");
        break;
    }
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      return { success: false, message: "Erro ao cadastrar restaurante. Tente novamente." };
    } finally {
